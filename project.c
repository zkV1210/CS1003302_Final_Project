...
int main(){
    ...
    char lenstring[10]={0},*trash,widthstring[15][100];
    int widthsf[25][6],widthsb[25][6],in[150],ansf[25],ansb[25];
    int length = 1, input = 0,f=0,b=0,casei=1;
    while(length!=0){
        for (int i = 0;i<25;i++){
            for (int n = 0;n<6;n++){
                widthsf[i][n] = -1;
        ...
        fgets(lenstring,10,file);
        length = strtol(lenstring,&trash,0);
        if(length==0){
            break;
        }
        printf("Case %d: ",casei);
        if (length%10!=0){
            for (int i = 0;i<length/10+1;i++){
                fgets(widthstring[i],100,file);
                in[i*10]=strtol(widthstring[i],&trash,0);
                for (int n = 1;n<10;n++){
                    if (i*10+n<length){
                        in[i*10+n]=strtol(trash,&trash,0);
                    }
                }
            }
        }
...
int arraystandardization(int array[],int len){
    float narrow = 0,a=0,b=0,count = 0;
    int now =0;
    while (narrow==0){
        a = array[now];
        b = array[now+1];
...
int checkCK(int ans[],int len){
    ...
    if(C2==ans[len-2]&&K2==ans[len-1]){
        return 2;
    }else if(C3==10&&ans[len-2]==0&&ans[len-3]==1&&K3==ans[len-1]){
        return 3;
    }else if(K3==10&&ans[len-1]==0&&ans[len-2]==1&&C3==ans[len-3]){
        return 3;
        ...
        return 1;
    }else if(C3==ans[len-3]&&ans[len-1]==0&&ans[len-2]==1){
        return 1;
    }else if(C4==10&&ans[len-3]==0&&ans[len-4]==1&&ans[len-1]==0&&ans[len-2]==1){
        return 1;
    }else {
        return 0;
    }
